...
// Click en los costados de la imagen
function manejarClickLateral(event, id) {
  const contenedor = event.currentTarget;
  const ancho = contenedor.offsetWidth;
  const clickX = event.offsetX;

  if (clickX < ancho / 2) {
    cambiarImagenModal(id, -1); // izquierda
  } else {
    cambiarImagenModal(id, 1); // derecha
  }
}
...
function actualizarMiniaturasModal() {
  const miniaturas = document.querySelectorAll("#modal-miniaturas img");
  miniaturas.forEach((img, i) => {
    img.classList.toggle("activa", i === window.imagenActualModal);
  });
}

// Eventos táctiles (swipe + pinch zoom)
function agregarEventosTactiles() {
  ...
  imagen.addEventListener("touchmove", (e) => {
    if (e.touches.length === 2 && distanciaInicial !== null) {
      let nuevaDistancia = calcularDistancia(e.touches);
      escala = nuevaDistancia / distanciaInicial;
      imagen.style.transform = `scale(${escala})`;
    }
  }, { passive: true });

  imagen.addEventListener("touchend", (e) => {
    if (e.changedTouches.length === 1 && xInicial !== null) {
